...
describe('Bucket', function () {
    it('creates a bucket with a route', function () {
    ...
    it('generates correct cache key', function () {
        ...
        $key = $keyMethod->invoke($bucket);

        expect($key)->toContain('buckets:');
        expect($key)->toContain('123456789');
    });

    it('returns new ratelimit when cache is empty', function () {
        $route = new Route('GET', '/test/path');
        $bucket = new Bucket($route);
    ...
    it('stores and retrieves ratelimit from cache', function () {
        $route = new Route('GET', '/test/path');
        $bucket = new Bucket($route);
    ...
    it('forgets ratelimit from cache', function () {
        $route = new Route('GET', '/test/path');
        $bucket = new Bucket($route);
    ...
    it('handles bucket with major parameters', function () {
    ...
    it('handles webhook routes', function () {
    ...
    it('generates unique keys for different routes', function () {
        ...
        $key1 = $keyMethod->invoke($bucket1);
        $key2 = $keyMethod->invoke($bucket2);

        expect($key1)->not->toBe($key2);
    });

    it('handles routes without major parameters', function () {
        $route = new Route('GET', '/test/path');
        $bucket = new Bucket($route);
