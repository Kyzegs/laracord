...
resolveFacades($finder)->each(function ($facade) use ($linting) {
     ...
     *
    {$proxies->map(fn ($class) => " * @see {$class}")->merge(
        $proxies->isNotEmpty() && $directMixins->isNotEmpty() ? [' *'] : []
    )->merge($directMixins->map(fn ($class) => " * @mixin {$class}"))->join(PHP_EOL)}
     */
    PHP;
...
function resolveFacades($finder)
{
    return collect($finder)
        ...
        ->map(fn ($name) => "\\Kyzegs\\Laracord\\Facades\\{$name}")
...
function resolveDocblockTypes($method, $typeNode)
{
    ...
    if ($typeNode instanceof IdentifierTypeNode) {
        ...
        $guessedFqcn = resolveClassImports($method->getDeclaringClass())->get(
            $typeNode->name
        ) ?? '\\'.$method->getDeclaringClass()->getNamespaceName().'\\'.$typeNode->name;
...
function resolveClassImports($class)
{
    return Str::of(file_get_contents($class->getFileName()))
        ...
        ->mapWithKeys(fn ($class) => [
            ($class->contains(' as ') ? $class->after(' as ') : $class->classBasename())->toString() => $class->start(
                '\\'
            )->before(' as ')->toString(),
        ]);
}
